$(function() {
	$("#cargoresourcetable").jqGrid({
		caption: "货源信息",	//定义表格名称
		url:'cargoresourceController.do?loadDataGrid',
		datatype: 'json',   //表格可以被接受的数据类型：xml，xmlstring，json，local，function 
		height:500,		//表格高度，可以是数字，像素值或者百分比
		colNames:
		 ['id', '启运地','目的地','车辆要求','货物名称','货物规格','发布时间 ','事件有效截止日期','联系人名称','电话号码 '],  //放置列名称的数组，必须与colModel大小相同
	    colModel:[     // 描述列信息的数组 
				...
				{name:'vehicleRequireDescription',index: 'vehicleRequireDescription',width:$(this).width() * 0.067, align:'center'},
				{name:'descriptionOfGoods',index: 'descriptionOfGoods',width:$(this).width() * 0.067, align:'center'},
				{name:'goodsSpecification',index: 'goodsSpecification',width:$(this).width() * 0.067, align:'center'},
				...
				{name:'contactName',index: 'contactName',width:$(this).width() * 0.067, align:'center'},
				{name:'telephoneNumber',index: 'telephoneNumber',width:$(this).width() * 0.067, align:'center'}
				], 
		...
		pager : "#cargoresourcepager",	//指定分页栏对象，必须为一个有效的html元素。可以是'pager', '#pager', jQuery('#pager').推荐用'#pager'
		altRows: true,			//设置表格条纹
	...
	//设置操作按钮和样式
	$("#cargoresourcetable").jqGrid('navGrid',"#cargoresourcepager",
		{ 	
			edit: true,
...
function seachData(){
	...
	$("#cargoresourcetable").jqGrid('setGridParam',{ 
        datatype:'json', 
        postData:{
...
function addFun(){
	$("#addDiv").modal({
		    remote: "cargoresourceController.do?editToView"
	});
}
...
function editFun(id){
	$("#addDiv").modal({
	    remote: "cargoresourceController.do?editToView&id=" + id
	});
}
...
//删除
function delFun(id){
	var url = "cargoresourceController.do?del";
...
function showmsg(data){
	if(data==1){
	   ...
	   $("#cargoresourcetable").trigger("reloadGrid");  //刷新表格
	}else{
	   $.jqalert("操作失败！");
	   $('#addDiv').modal('hide');	//关闭弹出窗
	   $("#cargoresourcetable").trigger("reloadGrid");  //刷新表格
